document.write('<link rel="stylesheet" type="text/css" href="modules/Web_tv_twitch/web_tv_v1.css">');
...
function check_url() {
     	...
     	xhr.open("POST","index.php?file=Web_tv_twitch&page=admin&nuked_nude=admin&op=check_url",true);
     	xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
     	var idurl  = document.getElementById('url').value;
        a = document.getElementById('type');
     	xhr.send("url="+encodeURIComponent(idurl)+"");
}
     	...
     	xhr.onreadystatechange = function() {
          	if(xhr.readyState == 4) {
	        	if(xhr.status == 200) {
	       		...
	       		document.getElementById('hpt').innerHTML = "<img src='modules/Web_tv_twitch/images/loading.gif' alt=''/>"
	   	}
     	}
        xhr.open("GET","index.php?file=Web_tv_twitch&nuked_nude=index&op=historique_programme&id="+id_tv+"&p="+page,true);
        xhr.send(null)}